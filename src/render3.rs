...
pub fn draw(f: &mut Frame, g: &mut Game3) {
    ...
    if g.help_open {
        let movement: Vec<(&str, &str)> = if g.creative {
            vec![
                ("Movement", ""),
...
fn draw_hud(f: &mut Frame, g: &Game3, area: Rect, hud_h: u16) {
    ...
    let help = if g.creative {
        if g.is_multiplayer() {
            "h help  F2 mode  e inv  w/a/s/d fly  space up  f down  x mine  z place  c craft  t chat  Tab players  q quit"
        } else {
            "h help  F2 mode  e inv  w/a/s/d fly  ←↑→↓ look  space up  f down  x mine  z place  c craft  q quit"
        }
    } else if g.is_multiplayer() {
        "h help  F2 mode  e inv  w/a/s/d move  x mine  z place  c craft  t chat  Tab players  q quit"
    } else {
        "h help  F2 mode  e inv  w/a/s/d move  ←↑→↓ look  space jump  x mine  z place  c craft  q quit"
    };
    f.render_widget(
